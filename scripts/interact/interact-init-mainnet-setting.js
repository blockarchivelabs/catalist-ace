...
async function main() {
  ...
  const HashConsensusForAccountingOracleAddress =
    addresses.hashConsensusForAccountingOracle.address;
  const HashConsensusForForValidatorsExitBusOracle =
    addresses.hashConsensusForValidatorsExitBusOracle.address;
  ...
  const ValidatorsExitBusOracleAddress =
    addresses.validatorsExitBusOracle.proxy.address;
  ...
  const OracleReportSanityCheckerAddress =
    addresses.oracleReportSanityChecker.address;
  ...
  const catalistProxy = await ethers.getContractAt(
    'AppProxyUpgradeable',
    CatalistAddress,
  );
  const catalistAragonApp = await ethers.getContractAt(
    'AragonApp',
    CatalistAddress,
  );
  const hashConsensusForAccountingOracle = await ethers.getContractAt(
    'HashConsensus',
    HashConsensusForAccountingOracleAddress,
  );
  const hashConsensusForValidatorsExitBusOracle = await ethers.getContractAt(
    'HashConsensus',
    HashConsensusForForValidatorsExitBusOracle,
  );
  ...
  const catalistLocator = await ethers.getContractAt(
    'CatalistLocator',
    CatalistLocatorAddress,
  );
  const catalistLocatorProxy = await ethers.getContractAt(
    'OssifiableProxy',
    CatalistLocatorAddress,
  );
  ...
  const aragonKernel = await ethers.getContractAt(
    'Kernel',
    AragonKernelAddress,
  );
  const aragonKernelProxy = await ethers.getContractAt(
    'KernelProxy',
    AragonKernelAddress,
  );
  const aragonAcl = await ethers.getContractAt('ACL', AragonAclAddress);
  const oracleReportSanityChecker = await ethers.getContractAt(
  ...
  console.log();
  console.log(
    'Grant MANAGE_MEMBERS_AND_QUORUM_ROLE to deployer in HashConsensus...',
  );
  await hashConsensusForAccountingOracle.grantRole(
    await hashConsensusForAccountingOracle.MANAGE_MEMBERS_AND_QUORUM_ROLE(
      GAS_INFO,
    ),
    deployerAddress,
    GAS_INFO,
  ...
  await hashConsensusForValidatorsExitBusOracle.grantRole(
    await hashConsensusForValidatorsExitBusOracle.MANAGE_MEMBERS_AND_QUORUM_ROLE(
      GAS_INFO,
    ),
    deployerAddress,
    GAS_INFO,
  ...
  const latestBlockTimestamp = (await ethers.provider.getBlock('latest'))
    .timestamp;
  let initialEpoch = Math.floor(
    (latestBlockTimestamp - GENESIS_TIME) /
  ...
  await depositSecurityModule.addGuardian(oracleMemberAddress, 1, GAS_INFO);
  ...
  await stakingRouter.resumeStakingModule(STAKING_MODULE_ID, GAS_INFO);
  ...
  console.log(
    'Grant MANAGE_NODE_OPERATOR_ROLE to owner in NodeOperatorsRegistry...',
  );
  const MANAGE_NODE_OPERATOR_ROLE =
    await nodeOperatorRegistry.MANAGE_NODE_OPERATOR_ROLE(GAS_INFO);
  await aragonAcl.grantPermission(
    deployerAddress,
    NodeOperatorRegistryAddress,
