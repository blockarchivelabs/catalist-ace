...
async function deployTemplate({ web3, artifacts }) {
  ...
  logHeader('DAO template')
  {
    ...
    const daoTemplate = await artifacts.require('CatalistTemplate').at(daoTemplateAddress)
    log(`Checking...`)
    await assertDeployedBytecode(daoTemplate.address, 'CatalistTemplate')
  ...
  logHeader('Catalist app base')
  {
    const catalistBaseAddress = state[`app:${APP_NAMES.CATALIST}`].implementation.address
    const catalistBase = await artifacts.require('Catalist').at(catalistBaseAddress)
    log(`Checking...`)
    await assertDeployedBytecode(catalistBaseAddress, 'Catalist')
    await assertAragonProxyBase(catalistBase, 'catalistBase')
  }
