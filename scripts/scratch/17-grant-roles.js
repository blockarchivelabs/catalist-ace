...
async function deployNewContracts({ web3, artifacts }) {
  ...
  await makeTx(stakingRouter, 'grantRole', [await stakingRouter.REPORT_REWARDS_MINTED_ROLE(), catalistAddress], { from: deployer })
  logWideSplitter()
  ...
  await makeTx(withdrawalQueue, 'grantRole', [await withdrawalQueue.FINALIZE_ROLE(), catalistAddress], { from: deployer })
  ...
  // NB: REQUEST_BURN_SHARES_ROLE is already granted to Catalist in Burner constructor
