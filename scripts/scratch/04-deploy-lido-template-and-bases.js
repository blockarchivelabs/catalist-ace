...
async function deployTemplate({ web3, artifacts }) {
  ...
  await deployWithoutProxy('catalistTemplate', 'CatalistTemplate', state.deployer, daoTemplateConstructorArgs)
  const daoTemplateDeployBlock = (await ethers.provider.getBlock('latest')).number

  await deployImplementation(`app:${APP_NAMES.CATALIST}`, 'Catalist', state.deployer)
