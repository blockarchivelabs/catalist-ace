...
async function deployTemplate({ web3, artifacts }) {
  ...
  const domainName = state.catalistApmEnsName
  const domainOwner = state.catalistTemplate.address
  const domainRegDuration = state.catalistApmEnsRegDurationSec
  ...
  if ((await ens.owner(node)) !== state.deployer && (await ens.owner(tldNode)) !== state.deployer) {
    ...
    const controller = await artifacts.require('IACERegistrarController').at(controllerAddr)
    ...
    log(`Rent price:`, yl(`${web3.utils.fromWei(rentPrice, 'ether')} ACE`))
    ...
    log(`Register TX value:`, yl(`${web3.utils.fromWei(registerTxValue, 'ether')} ACE`))
