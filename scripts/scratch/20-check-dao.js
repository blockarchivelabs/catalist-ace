...
const REQUIRED_NET_STATE = ['ens', 'catalistApmEnsName', 'daoAragonId', 'vestingParams', 'daoInitialSettings', 'catalistTemplate']
...
const BACE_TOKEN_NAME = 'Bond Ace 2.0'
...
async function checkDAO({ web3, artifacts }) {
  ...
  log(`Using CatalistTemplate: ${yl(daoTemplateAddress)}`)
  ...
  await assertLastEvent(template, 'TmplDaoFinalized', null, state.catalistTemplate.deployBlock)

  const apmDeployedEvt = await assertSingleEvent(template, 'TmplAPMDeployed', null, state.catalistTemplate.deployBlock)
  const daoDeployedEvt = await assertSingleEvent(template, 'TmplDAOAndTokenDeployed', null, state.catalistTemplate.deployBlock)

  catalistApmAddress = apmDeployedEvt.args.apm
  daoAddress = daoDeployedEvt.args.dao
  daoTokenAddress = daoDeployedEvt.args.token

  log.splitter()

  log(`Using APMRegistry:`, yl(catalistApmAddress))
  const registry = await artifacts.require('APMRegistry').at(catalistApmAddress)
  ...
  const [catalist, legacyOracle, nopsRegistry, agent, finance, tokenManager, voting, burner, elRewardsVault, stakingRouter] = await Promise.all([
    artifacts.require('Catalist').at(apps[APP_NAMES.CATALIST].proxyAddress),
    ...
    artifacts.require('CatalistExecutionLayerRewardsVault').at(state.executionLayerRewardsVault["address"]),
  ...
  const { registryACL } = await assertCatalistAPMPermissions({ registry, votingAddress: voting.address }, state.catalistTemplate.deployBlock)

  log.splitter()

  await assertReposPermissions({ registry, registryACL, votingAddress: voting.address }, state.catalistTemplate.deployBlock)
...
async function assertCatalistAPMPermissions({ registry, votingAddress }, fromBlock = 4532202) {
...
async function assertDAOConfig({
  ...
  DAO_LIVE ||
    assert.log(assert.equal, await catalist.isStopped(), PROTOCOL_PAUSED_AFTER_DEPLOY, `catalist.isStopped is ${yl(PROTOCOL_PAUSED_AFTER_DEPLOY)}`)

  DAO_LIVE ||
    assert.log(
      assert.hexEqual,
      await catalist.getWithdrawalCredentials(),
      `0x010000000000000000000000${state.withdrawalVault.address.slice(2)}`,
      `catalist.getWithdrawalCredentials() is ${yl(await catalist.getWithdrawalCredentials())}`
    )
  ...
  assert.log(assert.addressEqual, await catalist.getOracle(), legacyOracle.address, `catalist.getOracle() is ${yl(legacyOracle.address)}`)


  assert.log(assert.addressEqual, await catalist.getTreasury(), agent.address, `catalist.getTreasury() is ${yl(agent.address)}`)

  log.splitter()
  await assertKernel(legacyOracle, 'oracle')

  assert.log(assert.addressEqual, await legacyOracle.getCatalist(), catalist.address, `legacyOracle.getCatalist() is ${yl(catalist.address)}`)
...
async function assertAragonPermissions({ kernel, catalist, legacyOracle, nopsRegistry, agent, finance, tokenManager, voting, burner, stakingRouter }, fromBlock = 4532202) {
