...
contract WithdrawalsManagerStub {
     ...
     * @dev Receives Ace.
     *
     * Currently this is intentionally not supported since Ace 2.0 withdrawals specification
     ...
     *
     * When Ace 2.0 withdrawals specification is finalized, Catalist DAO will prepare the new
...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an ACE balance of at least `value`.
...
/**
 * @dev A proxy for Catalist Ace 2.0 withdrawals manager contract.
 *
 ...
 * are enabled in the Merge network. This means that Catalist cannot deploy the final implementation
 ...
 *
 * To solve this, Catalist uses an upgradeable proxy controlled by the DAO. Initially, it uses a stub
 * implementation contract, WithdrawalsManagerStub, that cannot receive Ace. The implementation
 * can only be changed by LDO holders collectively by performing a DAO vote. Catalist will set validator
 ...
 * new implementation.
 *
 * Once withdrawals are enabled in Ace 2.0, Catalist DAO members will start a vote among LDO
...
contract WithdrawalsManagerProxy is ERC1967Proxy {
    /**
     * @dev The address of Catalist DAO Voting contract.
     */
    address internal immutable CATALIST_VOTING;
