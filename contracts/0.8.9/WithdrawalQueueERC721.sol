...
contract WithdrawalQueueERC721 is IERC721Metadata, IERC4906, WithdrawalQueue {
    ...
    bytes32 internal constant TOKEN_APPROVALS_POSITION =
        keccak256("catalist.WithdrawalQueueERC721.tokenApprovals");
    bytes32 internal constant OPERATOR_APPROVALS_POSITION =
        keccak256("catalist.WithdrawalQueueERC721.operatorApprovals");
    bytes32 internal constant BASE_URI_POSITION =
        keccak256("catalist.WithdrawalQueueERC721.baseUri");
    bytes32 internal constant NFT_DESCRIPTOR_ADDRESS_POSITION =
        keccak256("catalist.WithdrawalQueueERC721.nftDescriptorAddress");
    ...
    function tokenURI(
        ...
        address nftDescriptorAddress = NFT_DESCRIPTOR_ADDRESS_POSITION
            .getStorageAddress();
        if (nftDescriptorAddress != address(0)) {
            return
                INFTDescriptor(nftDescriptorAddress).constructTokenURI(
                    _requestId
                );
        } else {
    ...
    function setNFTDescriptorAddress(
        ...
        NFT_DESCRIPTOR_ADDRESS_POSITION.setStorageAddress(
            _nftDescriptorAddress
        );
    ...
    function _transfer(
        ...
        if (
            !(_from == msgSender ||
                ...
                _getTokenApprovals()[_requestId] == msgSender)
        ) {
