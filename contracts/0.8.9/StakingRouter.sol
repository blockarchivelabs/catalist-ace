    ...
    Versioned
{
    ...
    /// Emitted when the StakingRouter received ACE
    event StakingRouterACEDeposited(
        uint256 indexed stakingModuleId,
        uint256 amount
    );
    ...
    bytes32 public constant STAKING_MODULE_RESUME_ROLE =
        keccak256("STAKING_MODULE_RESUME_ROLE");
    bytes32 public constant STAKING_MODULE_MANAGE_ROLE =
        keccak256("STAKING_MODULE_MANAGE_ROLE");
    bytes32 public constant REPORT_EXITED_VALIDATORS_ROLE =
        keccak256("REPORT_EXITED_VALIDATORS_ROLE");
    bytes32 public constant UNSAFE_SET_EXITED_VALIDATORS_ROLE =
        keccak256("UNSAFE_SET_EXITED_VALIDATORS_ROLE");
    bytes32 public constant REPORT_REWARDS_MINTED_ROLE =
        keccak256("REPORT_REWARDS_MINTED_ROLE");

    bytes32 internal constant CATALIST_POSITION =
        keccak256("catalist.StakingRouter.catalist");

    /// @dev Credentials which allows the DAO to withdraw Ace on the 2.0 side
    bytes32 internal constant WITHDRAWAL_CREDENTIALS_POSITION =
        keccak256("catalist.StakingRouter.withdrawalCredentials");

    /// @dev total count of staking modules
    bytes32 internal constant STAKING_MODULES_COUNT_POSITION =
        keccak256("catalist.StakingRouter.stakingModulesCount");
    /// @dev id of the last added staking module. This counter grow on staking modules adding
    bytes32 internal constant LAST_STAKING_MODULE_ID_POSITION =
        keccak256("catalist.StakingRouter.lastStakingModuleId");
    /// @dev mapping is used instead of array to allow to extend the StakingModule
    bytes32 internal constant STAKING_MODULES_MAPPING_POSITION =
        keccak256("catalist.StakingRouter.stakingModules");
    ...
    bytes32 internal constant STAKING_MODULE_INDICES_MAPPING_POSITION =
        keccak256("catalist.StakingRouter.stakingModuleIndicesOneBased");
     ...
     * @param _admin Catalist DAO Aragon agent contract address
     * @param _catalist Catalist address
     * @param _withdrawalCredentials Catalist withdrawal vault contract address
     */
    function initialize(
        address _admin,
        address _catalist,
        ...
        if (_catalist == address(0)) revert ZeroAddress("_catalist");

        _initializeContractVersionTo(1);

        _setupRole(DEFAULT_ADMIN_ROLE, _admin);

        CATALIST_POSITION.setStorageAddress(_catalist);
        WITHDRAWAL_CREDENTIALS_POSITION.setStorageBytes32(
    ...
    /**
     * @notice Return the Catalist contract address
     */
    ...
    function addStakingModule(
        ...
        if (
            ...
            bytes(_name).length > MAX_STAKING_MODULE_NAME_LENGTH
        ...
        for (uint256 i; i < newStakingModuleIndex; ) {
            if (
                ...
                _getStakingModuleByIndex(i).stakingModuleAddress
        ...
        StakingModule storage newStakingModule = _getStakingModuleByIndex(
            newStakingModuleIndex
        );
        uint24 newStakingModuleId = uint24(
            LAST_STAKING_MODULE_ID_POSITION.getStorageUint256()
        ) + 1;
        ...
        emit StakingRouterACEDeposited(newStakingModuleId, 0);
        ...
        STAKING_MODULES_COUNT_POSITION.setStorageUint256(
            newStakingModuleIndex + 1
        );

        emit StakingModuleAdded(
            newStakingModuleId,
            _stakingModuleAddress,
            _name,
            msg.sender
        );
        emit StakingModuleTargetShareSet(
            newStakingModuleId,
            _targetShare,
            msg.sender
        );
        emit StakingModuleFeesSet(
            newStakingModuleId,
            _stakingModuleFee,
            _treasuryFee,
            msg.sender
    ...
    function updateTargetValidatorsLimits(
        ...
        address moduleAddr = _getStakingModuleById(_stakingModuleId)
            .stakingModuleAddress;
        IStakingModule(moduleAddr).updateTargetValidatorsLimits(
            _nodeOperatorId,
            _isTargetLimitActive,
            _targetLimit
        );
    }
    ...
    function updateRefundedValidatorsCount(
        ...
        address moduleAddr = _getStakingModuleById(_stakingModuleId)
            .stakingModuleAddress;
        IStakingModule(moduleAddr).updateRefundedValidatorsCount(
            _nodeOperatorId,
    ...
    function reportRewardsMinted(
        ...
        for (uint256 i = 0; i < _stakingModuleIds.length; ) {
            if (_totalShares[i] > 0) {
                address moduleAddr = _getStakingModuleById(_stakingModuleIds[i])
                    .stakingModuleAddress;
                try
                    IStakingModule(moduleAddr).onRewardsMinted(_totalShares[i])
                {} catch (bytes memory lowLevelRevertData) {
                    /// @dev This check is required to prevent incorrect gas estimation of the method.
                    ///      Without it, Ace nodes that use binary search for gas estimation may
                    ...
                    if (lowLevelRevertData.length == 0)
                        revert UnrecoverableModuleError();
    ...
    function updateExitedValidatorsCountByStakingModule(
        ...
        for (uint256 i = 0; i < _stakingModuleIds.length; ) {
            ...
            uint256 prevReportedExitedValidatorsCount = stakingModule
                .exitedValidatorsCount;
            if (
                _exitedValidatorsCounts[i] < prevReportedExitedValidatorsCount
            ) {
            ...
            newlyExitedValidatorsCount +=
                _exitedValidatorsCounts[i] -
                prevReportedExitedValidatorsCount;
    ...
    function reportStakingModuleExitedValidatorsCountByNodeOperator(
        ...
        address moduleAddr = _getStakingModuleById(_stakingModuleId)
            .stakingModuleAddress;
        _checkValidatorsByNodeOperatorReportData(
            _nodeOperatorIds,
    ...
    function unsafeSetExitedValidatorsCount(
        ...
        if (
            ...
            _correction.currentNodeOperatorStuckValidatorsCount !=
        ...
        stakingModule.exitedValidatorsCount = _correction
        ...
        onlyRole(REPORT_EXITED_VALIDATORS_ROLE)
    {
        ...
        for (uint256 i; i < stakingModulesCount; ) {
            ...
            IStakingModule moduleContract = IStakingModule(
                stakingModule.stakingModuleAddress
            );

            (uint256 exitedValidatorsCount, , ) = moduleContract
                .getStakingModuleSummary();
    ...
    function getStakingModule(
        uint256 _stakingModuleId
    ) public view returns (StakingModule memory) {
    ...
    function getStakingModuleStatus(
        uint256 _stakingModuleId
    ) public view returns (StakingModuleStatus) {
        return
            StakingModuleStatus(_getStakingModuleById(_stakingModuleId).status);
    }
    ...
    struct NodeOperatorSummary {
        ...
        ///     compensated to the Catalist by the node operator
    ...
    function getStakingModuleSummary(
        ...
        StakingModule memory stakingModuleState = getStakingModule(
            _stakingModuleId
        );
        IStakingModule stakingModule = IStakingModule(
            stakingModuleState.stakingModuleAddress
        );
        (
    ...
    function getStakingModuleDigests(
        ...
        for (uint256 i = 0; i < _stakingModuleIds.length; ++i) {
            ...
            digests[i] = StakingModuleDigest({
                ...
                activeNodeOperatorsCount: stakingModule
    ...
    function getAllNodeOperatorDigests(
        ...
        IStakingModule stakingModule = IStakingModule(
            _getStakingModuleAddressById(_stakingModuleId)
        );
    ...
    function getNodeOperatorDigests(
        ...
        uint256[] memory nodeOperatorIds = stakingModule.getNodeOperatorIds(
            _offset,
            _limit
        );
    ...
    function getNodeOperatorDigests(
        ...
        for (uint256 i = 0; i < _nodeOperatorIds.length; ++i) {
            digests[i] = NodeOperatorDigest({
                ...
                isActive: stakingModule.getNodeOperatorIsActive(
                    _nodeOperatorIds[i]
                ),
                summary: getNodeOperatorSummary(
                    _stakingModuleId,
                    _nodeOperatorIds[i]
                )
            });
    ...
    function pauseStakingModule(
        uint256 _stakingModuleId
    ) external onlyRole(STAKING_MODULE_PAUSE_ROLE) {
        StakingModule storage stakingModule = _getStakingModuleById(
            _stakingModuleId
        );
        if (
            StakingModuleStatus(stakingModule.status) !=
            StakingModuleStatus.Active
        ) revert StakingModuleNotActive();
        _setStakingModuleStatus(
            stakingModule,
    ...
    function resumeStakingModule(
        uint256 _stakingModuleId
    ) external onlyRole(STAKING_MODULE_RESUME_ROLE) {
        StakingModule storage stakingModule = _getStakingModuleById(
            _stakingModuleId
        );
        if (
    ...
    function getStakingModuleIsStopped(
        ...
        return
            getStakingModuleStatus(_stakingModuleId) ==
    ...
    function getStakingModuleNonce(
        ...
        return
            IStakingModule(_getStakingModuleAddressById(_stakingModuleId))
                .getNonce();
    }
    ...
    function getStakingModuleMaxDepositsCount(
        ...
        uint256 stakingModuleIndex = _getStakingModuleIndexById(
            _stakingModuleId
        );
        return
            newDepositsAllocation[stakingModuleIndex] -
            stakingModulesCache[stakingModuleIndex].activeValidatorsCount;
    }
        ...
        )
    {
        ...
        for (uint256 i; i < stakingModulesCount; ) {
            ...
            if (stakingModulesCache[i].activeValidatorsCount > 0) {
                ...
                ] = stakingModulesCache[i].stakingModuleId;
                stakingModuleValidatorsShare = ((stakingModulesCache[i]
                    .activeValidatorsCount * precisionPoints) /
                    totalActiveValidators);

                recipients[rewardedStakingModulesCount] = address(
                    stakingModulesCache[i].stakingModuleAddress
                );
                stakingModuleFee = uint96(
                    (stakingModuleValidatorsShare *
                        stakingModulesCache[i].stakingModuleFee) /
                        TOTAL_BASIS_POINTS
                );
                ...
                if (
                    stakingModulesCache[i].status != StakingModuleStatus.Stopped
                ) {
                    stakingModuleFees[
                        rewardedStakingModulesCount
                    ] = stakingModuleFee;
                ...
                totalFee += (uint96(
                    (stakingModuleValidatorsShare *
                        stakingModulesCache[i].treasuryFee) / TOTAL_BASIS_POINTS
                ) + stakingModuleFee);
    ...
    /// @notice Helper for Catalist contract (DEPRECATED)
    ...
    function getTotalFeeE4Precision() external view returns (uint16 totalFee) {
        /// @dev The logic is placed here but in Catalist contract to save Catalist bytecode
        (
            ,
            ,
            ,
            uint96 totalFeeInHighPrecision,
            uint256 precision
        ) = getStakingRewardsDistribution();
    ...
    /// @dev Helper only for Catalist contract. Use getStakingFeeAggregateDistribution() instead
    ...
    function deposit(
        ...
        if (msg.sender != CATALIST_POSITION.getStorageAddress())
            revert AppAuthCatalistFailed();
        ...
        emit StakingRouterACEDeposited(_stakingModuleId, depositsValue);
    ...
    /**
     * @notice Set credentials to withdraw ACE on Consensus Layer side after the phase 2 is launched to `_withdrawalCredentials`
     * @dev Note that setWithdrawalCredentials discards all unused deposits data as the signatures are invalidated.
     * @param _withdrawalCredentials withdrawal credentials field as defined in the Ace PoS consensus specs
     */
    function setWithdrawalCredentials(
        ...
        for (uint256 i; i < stakingModulesCount; ) {
            ...
            {} catch (bytes memory lowLevelRevertData) {
                ...
                emit WithdrawalsCredentialsChangeFailed(
                    stakingModule.id,
                    lowLevelRevertData
                );
            }
    ...
    /**
     * @notice Returns current credentials to withdraw ACE on Consensus Layer side after the phase 2 is launched
     */
    ...
    function _checkValidatorsByNodeOperatorReportData(
        ...
        if (
            _nodeOperatorIds.length % 8 != 0 ||
            _validatorsCounts.length % 16 != 0
        ) {
        ...
        )
    {
        ...
        for (uint256 i; i < stakingModulesCount; ) {
            ...
            totalActiveValidators += stakingModulesCache[i]
                .activeValidatorsCount;
            unchecked {
                ++i;
            }
    ...
    function _loadStakingModulesCacheItem(
        ...
        StakingModule storage stakingModuleData = _getStakingModuleByIndex(
            _stakingModuleIndex
        );
        ...
        (
            ...
            uint256 totalDepositedValidators,
            uint256 depositableValidatorsCount
        ) = IStakingModule(cacheItem.stakingModuleAddress)
                .getStakingModuleSummary();

        cacheItem.availableValidatorsCount = cacheItem.status ==
            StakingModuleStatus.Active
    ...
    function _setStakingModuleStatus(
        StakingModule storage _stakingModule,
        StakingModuleStatus _status
    ) internal {
        StakingModuleStatus prevStatus = StakingModuleStatus(
            _stakingModule.status
        );
        ...
        )
    {
        ...
        if (stakingModulesCount > 0) {
            ...
            for (uint256 i; i < stakingModulesCount; ) {
                ...
                targetValidators =
                    (stakingModulesCache[i].targetShare *
                        totalActiveValidators) /
                    TOTAL_BASIS_POINTS;
                capacities[i] = Math256.min(
                    targetValidators,
                    stakingModulesCache[i].activeValidatorsCount +
                        stakingModulesCache[i].availableValidatorsCount
                );
                unchecked {
                    ++i;
                }
    ...
    function _getStakingModuleIndexById(
        uint256 _stakingModuleId
    ) internal view returns (uint256) {
        mapping(uint256 => uint256)
            storage _stakingModuleIndicesOneBased = _getStorageStakingIndicesMapping();
    ...
    function _setStakingModuleIndexById(
        uint256 _stakingModuleId,
        uint256 _stakingModuleIndex
    ) internal {
        mapping(uint256 => uint256)
            storage _stakingModuleIndicesOneBased = _getStorageStakingIndicesMapping();
        _stakingModuleIndicesOneBased[_stakingModuleId] =
            _stakingModuleIndex +
            1;
    }

    function _getStakingModuleById(
        uint256 _stakingModuleId
    ) internal view returns (StakingModule storage) {
        return
            _getStakingModuleByIndex(
                _getStakingModuleIndexById(_stakingModuleId)
            );
    }

    function _getStakingModuleByIndex(
        uint256 _stakingModuleIndex
    ) internal view returns (StakingModule storage) {
        mapping(uint256 => StakingModule)
            storage _stakingModules = _getStorageStakingModulesMapping();
        return _stakingModules[_stakingModuleIndex];
    }

    function _getStakingModuleAddressById(
        uint256 _stakingModuleId
    ...
    function _getStorageStakingIndicesMapping()
        internal
        pure
