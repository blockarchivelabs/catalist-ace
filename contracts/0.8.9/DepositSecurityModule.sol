...
contract DepositSecurityModule {
    ...
    constructor(
        ...
        if (_depositContract == address(0))
            revert ZeroAddress("_depositContract");
        if (_stakingRouter == address(0)) revert ZeroAddress("_stakingRouter");

        CATALIST = ICatalist(_catalist);
     ...
     */
    function getPauseIntentValidityPeriodBlocks()
        external
        view
        returns (uint256)
    {
    ...
    function _setPauseIntentValidityPeriodBlocks(uint256 newValue) internal {
        if (newValue == 0)
            revert ZeroParameter("pauseIntentValidityPeriodBlocks");
        pauseIntentValidityPeriodBlocks = newValue;
        emit PauseIntentValidityPeriodBlocksChanged(newValue);
    }

    /**
     * Returns `maxDepositsPerBlock` (see `depositBufferedAce`).
     */
    ...
    /**
     * Returns `minDepositBlockDistance`  (see `depositBufferedAce`).
     */
    ...
    function pauseDeposits(
        ...
        if (block.number - blockNumber > pauseIntentValidityPeriodBlocks)
            revert PauseIntentExpired();
    ...
    /**
     * Returns whether CATALIST.deposit() can be called, given that the caller will provide
    ...
    function canDeposit(uint256 stakingModuleId) external view returns (bool) {
        ...
        bool isModuleActive = STAKING_ROUTER.getStakingModuleIsActive(
            stakingModuleId
        );
        uint256 lastDepositBlock = STAKING_ROUTER
    ...
    /**
     * Calls CATALIST.deposit(maxDepositsPerBlock, stakingModuleId, depositCalldata).
     *
    ...
    function depositBufferedAce(
        ...
        uint256 lastDepositBlock = STAKING_ROUTER
            .getStakingModuleLastDepositBlock(stakingModuleId);
        if (block.number - lastDepositBlock < minDepositBlockDistance)
            revert DepositTooFrequent();
        if (blockHash == bytes32(0) || blockhash(blockNumber) != blockHash)
            revert DepositUnexpectedBlockHash();

        uint256 onchainNonce = STAKING_ROUTER.getStakingModuleNonce(
            stakingModuleId
        );
