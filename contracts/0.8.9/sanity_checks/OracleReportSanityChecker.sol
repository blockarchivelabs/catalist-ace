...
struct LimitsList {
    ...
    /// @notice The positive token rebase allowed per single CatalistOracle report
...
/// @title Sanity checks for the Catalist's oracle report
/// @notice The contracts contain view methods to perform sanity checks of the Catalist's oracle report
...
contract OracleReportSanityChecker is AccessControlEnumerable {
    ...
    ICatalistLocator private immutable CATALIST_LOCATOR;
    ...
    /// @param _catalistLocator address of the CatalistLocator instance
    ...
    constructor(
        ...
        CATALIST_LOCATOR = ICatalistLocator(_catalistLocator);
        ...
        _grantRole(
            ...
            _managersRoster.churnValidatorsPerDayLimitManagers
        );
        _grantRole(
        ...
        _grantRole(
            ...
            _managersRoster.annualBalanceIncreaseLimitManagers
        );
        _grantRole(
            MAX_POSITIVE_TOKEN_REBASE_MANAGER_ROLE,
            _managersRoster.maxPositiveTokenRebaseManagers
        );
        _grantRole(
        ...
        _grantRole(
            ...
            _managersRoster.shareRateDeviationLimitManagers
        );
        _grantRole(
            REQUEST_TIMESTAMP_MARGIN_MANAGER_ROLE,
            _managersRoster.requestTimestampMarginManagers
        );
    }

    /// @notice returns the address of the CatalistLocator
    function getCatalistLocator() public view returns (address) {
        return address(CATALIST_LOCATOR);
    }

    /// @notice Returns the limits list for the Catalist's oracle report sanity checks
    ...
    ///
    ///         preShareRate = preTotalPooledAce() / preTotalShares()
    ///         postShareRate = postTotalPooledAce() / postTotalShares()
    ...
    ///     (only for `activated` instead). For Catalist it's limited by the max daily deposits via DepositSecurityModule
    ///
    ...
    function setChurnValidatorsPerDayLimit(
        uint256 _churnValidatorsPerDayLimit
    ) external onlyRole(CHURN_VALIDATORS_PER_DAY_LIMIT_MANAGER_ROLE) {
    ...
    function setOneOffCLBalanceDecreaseBPLimit(
        uint256 _oneOffCLBalanceDecreaseBPLimit
    ) external onlyRole(ONE_OFF_CL_BALANCE_DECREASE_LIMIT_MANAGER_ROLE) {
        LimitsList memory limitsList = _limits.unpack();
        limitsList
            .oneOffCLBalanceDecreaseBPLimit = _oneOffCLBalanceDecreaseBPLimit;
        _updateLimits(limitsList);
    }
    ...
    function setSimulatedShareRateDeviationBPLimit(
        uint256 _simulatedShareRateDeviationBPLimit
    ) external onlyRole(SHARE_RATE_DEVIATION_LIMIT_MANAGER_ROLE) {
        LimitsList memory limitsList = _limits.unpack();
        limitsList
            .simulatedShareRateDeviationBPLimit = _simulatedShareRateDeviationBPLimit;
        _updateLimits(limitsList);
    }
    ...
    function setMaxExitRequestsPerOracleReport(
        ...
        LimitsList memory limitsList = _limits.unpack();
        limitsList
            .maxValidatorExitRequestsPerReport = _maxValidatorExitRequestsPerReport;
        _updateLimits(limitsList);
    }
    ...
    function setMaxAccountingExtraDataListItemsCount(
        uint256 _maxAccountingExtraDataListItemsCount
    ) external onlyRole(MAX_ACCOUNTING_EXTRA_DATA_LIST_ITEMS_COUNT_ROLE) {
        LimitsList memory limitsList = _limits.unpack();
        limitsList
            .maxAccountingExtraDataListItemsCount = _maxAccountingExtraDataListItemsCount;
        _updateLimits(limitsList);
    }
    ...
    function setMaxNodeOperatorsPerExtraDataItemCount(
        uint256 _maxNodeOperatorsPerExtraDataItemCount
    ) external onlyRole(MAX_NODE_OPERATORS_PER_EXTRA_DATA_ITEM_COUNT_ROLE) {
        LimitsList memory limitsList = _limits.unpack();
        limitsList
            .maxNodeOperatorsPerExtraDataItemCount = _maxNodeOperatorsPerExtraDataItemCount;
        _updateLimits(limitsList);
    }
        ...
        )
    {
        TokenRebaseLimiterData
            memory tokenRebaseLimiter = PositiveTokenRebaseLimiter
                .initLimiterState(
        ...
        if (_postCLBalance < _preCLBalance) {
            tokenRebaseLimiter.decreaseAce(_preCLBalance - _postCLBalance);
        } else {
            tokenRebaseLimiter.increaseAce(_postCLBalance - _preCLBalance);
        }

        withdrawals = tokenRebaseLimiter.increaseAce(_withdrawalVaultBalance);
        elRewards = tokenRebaseLimiter.increaseAce(_elRewardsVaultBalance);
        ...
        // after the off-chain calculation via `eth_call` of `Catalist.handleOracleReport()`
        // see also step 9 of the `Catalist._handleOracleReport()`
        simulatedSharesToBurn = Math256.min(
    ...
    /// @notice Applies sanity checks to the accounting params of Catalist's oracle report
    ...
    /// @param _preCLValidators Catalist-participating validators on the CL side before the current oracle report
    /// @param _postCLValidators Catalist-participating validators on the CL side after the current oracle report
    function checkAccountingOracleReport(
        ...
        address withdrawalVault = CATALIST_LOCATOR.withdrawalVault();
        // 1. Withdrawals vault reported balance
        _checkWithdrawalVaultBalance(
            withdrawalVault.balance,
            _withdrawalVaultBalance
        );

        address elRewardsVault = CATALIST_LOCATOR.elRewardsVault();
        // 2. EL rewards vault reported balance
        _checkELRewardsVaultBalance(
            elRewardsVault.balance,
        ...
        if (_postCLValidators > _preCLValidators) {
            _checkAppearedValidatorsChurnLimit(
                limitsList,
    ...
    function checkExitBusOracleReport(
        uint256 _exitRequestsCount
    ) external view {
    ...
    function checkExitedValidatorsRatePerDay(
    ...
    function checkNodeOperatorsPerExtraDataItemCount(
        ...
        if (_nodeOperatorsCount > limit) {
            revert TooManyNodeOpsPerExtraDataItem(
                _itemIndex,
                _nodeOperatorsCount
            );
        }
    ...
    function checkAccountingExtraDataListItemsCount(
        ...
        if (_extraDataListItemsCount > limit) {
            revert MaxAccountingExtraDataItemsCountExceeded(
                limit,
    ...
    function checkWithdrawalQueueOracleReport(
        ...
        LimitsList memory limitsList = _limits.unpack();
        address withdrawalQueue = CATALIST_LOCATOR.withdrawalQueue();

        _checkLastFinalizableId(
            limitsList,
            withdrawalQueue,
    ...
    function _checkSharesRequestedToBurn(
        uint256 _sharesRequestedToBurn
    ) internal view {
        (uint256 coverShares, uint256 nonCoverShares) = IBurner(
            CATALIST_LOCATOR.burner()
    ...
    function _checkOneOffCLBalanceDecrease(
        ...
        uint256 oneOffCLBalanceDecreaseBP = (MAX_BASIS_POINTS *
            (_preCLBalance - _unifiedPostCLBalance)) / _preCLBalance;
        if (
            oneOffCLBalanceDecreaseBP >
            _limitsList.oneOffCLBalanceDecreaseBPLimit
        ) {
    ...
    function _checkAppearedValidatorsChurnLimit(
        ...
        uint256 churnLimit = (_limitsList.churnValidatorsPerDayLimit *
            _timeElapsed) / SECONDS_PER_DAY;

        if (_appearedValidators > churnLimit)
            revert IncorrectAppearedValidators(_appearedValidators);
    }
    ...
    function _checkLastFinalizableId(
        ...
        IWithdrawalQueue.WithdrawalRequestStatus[]
            memory statuses = IWithdrawalQueue(_withdrawalQueue)
                .getWithdrawalStatus(requestIds);
        if (
            _reportTimestamp <
            statuses[0].timestamp + _limitsList.requestTimestampMargin
        ) revert IncorrectRequestFinalization(statuses[0].timestamp);
    }

    function _checkSimulatedShareRate(
        LimitsList memory _limitsList,
        uint256 _noWithdrawalsPostTotalPooledAce,
        uint256 _noWithdrawalsPostTotalShares,
        uint256 _simulatedShareRate
    ) internal pure {
        uint256 actualShareRate = (_noWithdrawalsPostTotalPooledAce *
            SHARE_RATE_PRECISION_E27) / _noWithdrawalsPostTotalShares;
        ...
        // - SRL is staking rate limit normalized by TVL (`maxStakeLimit / totalPooledAce`)
        //   totalPooledAce should be chosen as a reasonable lower bound of the protocol TVL
        //
        uint256 simulatedShareDiff = Math256.absDiff(
            actualShareRate,
            _simulatedShareRate
        );
        uint256 simulatedShareDeviation = (MAX_BASIS_POINTS *
            simulatedShareDiff) / actualShareRate;

        if (
            simulatedShareDeviation >
            _limitsList.simulatedShareRateDeviationBPLimit
        ) {
            revert IncorrectSimulatedShareRate(
                _simulatedShareRate,
                actualShareRate
            );
        }
    ...
    function _updateLimits(LimitsList memory _newLimitsList) internal {
        ...
        if (
            _oldLimitsList.churnValidatorsPerDayLimit !=
            _newLimitsList.churnValidatorsPerDayLimit
        ) {
            _checkLimitValue(
                _newLimitsList.churnValidatorsPerDayLimit,
                0,
            ...
            emit ChurnValidatorsPerDayLimitSet(
                _newLimitsList.churnValidatorsPerDayLimit
            );
        }
        if (
            _oldLimitsList.oneOffCLBalanceDecreaseBPLimit !=
            _newLimitsList.oneOffCLBalanceDecreaseBPLimit
        ) {
            _checkLimitValue(
                _newLimitsList.oneOffCLBalanceDecreaseBPLimit,
                0,
            ...
            emit OneOffCLBalanceDecreaseBPLimitSet(
                _newLimitsList.oneOffCLBalanceDecreaseBPLimit
            );
        }
        if (
            _oldLimitsList.annualBalanceIncreaseBPLimit !=
            _newLimitsList.annualBalanceIncreaseBPLimit
        ) {
            _checkLimitValue(
                _newLimitsList.annualBalanceIncreaseBPLimit,
                0,
            ...
            emit AnnualBalanceIncreaseBPLimitSet(
                _newLimitsList.annualBalanceIncreaseBPLimit
            );
        }
        if (
            _oldLimitsList.simulatedShareRateDeviationBPLimit !=
            _newLimitsList.simulatedShareRateDeviationBPLimit
        ) {
            _checkLimitValue(
                _newLimitsList.simulatedShareRateDeviationBPLimit,
                0,
            ...
            emit SimulatedShareRateDeviationBPLimitSet(
                _newLimitsList.simulatedShareRateDeviationBPLimit
            );
        }
        if (
            _oldLimitsList.maxValidatorExitRequestsPerReport !=
            _newLimitsList.maxValidatorExitRequestsPerReport
        ) {
            _checkLimitValue(
                _newLimitsList.maxValidatorExitRequestsPerReport,
                0,
            ...
            emit MaxValidatorExitRequestsPerReportSet(
                _newLimitsList.maxValidatorExitRequestsPerReport
            );
        }
        if (
            _oldLimitsList.maxAccountingExtraDataListItemsCount !=
            _newLimitsList.maxAccountingExtraDataListItemsCount
        ) {
            _checkLimitValue(
                _newLimitsList.maxAccountingExtraDataListItemsCount,
                0,
            ...
            emit MaxAccountingExtraDataListItemsCountSet(
                _newLimitsList.maxAccountingExtraDataListItemsCount
            );
        }
        if (
            _oldLimitsList.maxNodeOperatorsPerExtraDataItemCount !=
            _newLimitsList.maxNodeOperatorsPerExtraDataItemCount
        ) {
            _checkLimitValue(
                _newLimitsList.maxNodeOperatorsPerExtraDataItemCount,
                0,
            ...
            emit MaxNodeOperatorsPerExtraDataItemCountSet(
                _newLimitsList.maxNodeOperatorsPerExtraDataItemCount
            );
        }
        if (
            _oldLimitsList.requestTimestampMargin !=
            _newLimitsList.requestTimestampMargin
        ) {
            _checkLimitValue(
                _newLimitsList.requestTimestampMargin,
                0,
            ...
            emit RequestTimestampMarginSet(
                _newLimitsList.requestTimestampMargin
            );
        }
        if (
            _oldLimitsList.maxPositiveTokenRebase !=
            _newLimitsList.maxPositiveTokenRebase
        ) {
            _checkLimitValue(
                _newLimitsList.maxPositiveTokenRebase,
                1,
            ...
            emit MaxPositiveTokenRebaseSet(
                _newLimitsList.maxPositiveTokenRebase
            );
        }
    ...
    function _checkLimitValue(
        uint256 _value,
        uint256 _minAllowedValue,
        uint256 _maxAllowedValue
    ) internal pure {
        if (_value > _maxAllowedValue || _value < _minAllowedValue) {
            revert IncorrectLimitValue(
                _value,
                _minAllowedValue,
                _maxAllowedValue
            );
    ...
    event OneOffCLBalanceDecreaseBPLimitSet(
        uint256 oneOffCLBalanceDecreaseBPLimit
    );
    event AnnualBalanceIncreaseBPLimitSet(uint256 annualBalanceIncreaseBPLimit);
    event SimulatedShareRateDeviationBPLimitSet(
        uint256 simulatedShareRateDeviationBPLimit
    );
    event MaxPositiveTokenRebaseSet(uint256 maxPositiveTokenRebase);
    event MaxValidatorExitRequestsPerReportSet(
        uint256 maxValidatorExitRequestsPerReport
    );
    event MaxAccountingExtraDataListItemsCountSet(
        uint256 maxAccountingExtraDataListItemsCount
    );
    event MaxNodeOperatorsPerExtraDataItemCountSet(
        uint256 maxNodeOperatorsPerExtraDataItemCount
    );
    event RequestTimestampMarginSet(uint256 requestTimestampMargin);

    error IncorrectLimitValue(
        uint256 value,
        uint256 minAllowedValue,
        uint256 maxAllowedValue
    );
    error IncorrectWithdrawalsVaultBalance(
        uint256 actualWithdrawalVaultBalance
    );
    ...
    error IncorrectSimulatedShareRate(
        uint256 simulatedShareRate,
        uint256 actualShareRate
    );
    error MaxAccountingExtraDataItemsCountExceeded(
        uint256 maxItemsCount,
        uint256 receivedItemsCount
    );
    error ExitedValidatorsLimitExceeded(
        uint256 limitPerDay,
        uint256 exitedPerDay
    );
    error TooManyNodeOpsPerExtraDataItem(
        uint256 itemIndex,
...
library LimitsListPacker {
    function pack(
        LimitsList memory _limitsList
    ) internal pure returns (LimitsListPacked memory res) {
        res.churnValidatorsPerDayLimit = SafeCast.toUint16(
            _limitsList.churnValidatorsPerDayLimit
        );
        res.oneOffCLBalanceDecreaseBPLimit = _toBasisPoints(
            _limitsList.oneOffCLBalanceDecreaseBPLimit
        );
        res.annualBalanceIncreaseBPLimit = _toBasisPoints(
            _limitsList.annualBalanceIncreaseBPLimit
        );
        res.simulatedShareRateDeviationBPLimit = _toBasisPoints(
            _limitsList.simulatedShareRateDeviationBPLimit
        );
        res.requestTimestampMargin = SafeCast.toUint64(
            _limitsList.requestTimestampMargin
        );
        res.maxPositiveTokenRebase = SafeCast.toUint64(
            _limitsList.maxPositiveTokenRebase
        );
        res.maxValidatorExitRequestsPerReport = SafeCast.toUint16(
            _limitsList.maxValidatorExitRequestsPerReport
        );
        res.maxAccountingExtraDataListItemsCount = SafeCast.toUint16(
            _limitsList.maxAccountingExtraDataListItemsCount
        );
        res.maxNodeOperatorsPerExtraDataItemCount = SafeCast.toUint16(
            _limitsList.maxNodeOperatorsPerExtraDataItemCount
        );
    }
...
library LimitsListUnpacker {
    function unpack(
        LimitsListPacked memory _limitsList
    ) internal pure returns (LimitsList memory res) {
        res.churnValidatorsPerDayLimit = _limitsList.churnValidatorsPerDayLimit;
        res.oneOffCLBalanceDecreaseBPLimit = _limitsList
            .oneOffCLBalanceDecreaseBPLimit;
        res.annualBalanceIncreaseBPLimit = _limitsList
            .annualBalanceIncreaseBPLimit;
        res.simulatedShareRateDeviationBPLimit = _limitsList
        ...
        res.maxAccountingExtraDataListItemsCount = _limitsList
            .maxAccountingExtraDataListItemsCount;
        res.maxNodeOperatorsPerExtraDataItemCount = _limitsList
