...
abstract contract WithdrawalQueueBase {
    ...
    bytes32 internal constant REQUEST_BY_OWNER_POSITION =
        keccak256("catalist.WithdrawalQueue.requestsByOwner");
    /// @dev timestamp of the last oracle report
    bytes32 internal constant LAST_REPORT_TIMESTAMP_POSITION =
        keccak256("catalist.WithdrawalQueue.lastReportTimestamp");
    ...
    error ZeroAmountOfACE();
    error ZeroShareRate();
    error ZeroTimestamp();
    error TooMuchAceToFinalize(uint256 sent, uint256 maxExpected);
    ...
    ///  To start calculation you should pass `state.remainingAceBudget` and `state.finished == false` and then invoke
    ...
    function calculateFinalizationBatches(
        ...
        if (_state.finished || _state.remainingAceBudget == 0)
            revert InvalidState();
        ...
        while (currentId < queueLength && currentId < nextCallRequestId) {
            ...
            if (ethToFinalize > _state.remainingAceBudget) break; // budget break
            _state.remainingAceBudget -= ethToFinalize;
    ...
    function _finalize(
        ...
        _setLockedAceAmount(getLockedAceAmount() + _amountOfACE);
    ...
    /// @dev View function to find a checkpoint hint to use in `claimWithdrawal()` and `getClaimableAce()`
    ...
    function _claim(
        ...
        uint256 ethWithDiscount = _calculateClaimableAce(
            request,
            _requestId,
            _hint
        );
    ...
    function _calculateClaimableAce(
        ...
        uint256 _requestId,
        uint256 _hint
    ) internal view returns (uint256 claimableAce) {
