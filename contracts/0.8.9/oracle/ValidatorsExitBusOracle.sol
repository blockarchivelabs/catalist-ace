...
contract ValidatorsExitBusOracle is BaseOracle, PausableUntil {
    ...
    bytes32 internal constant TOTAL_REQUESTS_PROCESSED_POSITION =
        keccak256("catalist.ValidatorsExitBusOracle.totalRequestsProcessed");
    ...
    bytes32 internal constant LAST_REQUESTED_VALIDATOR_INDICES_POSITION =
        keccak256(
            "catalist.ValidatorsExitBusOracle.lastRequestedValidatorIndices"
        );

    /// @dev Storage slot: DataProcessingState dataProcessingState
    bytes32 internal constant DATA_PROCESSING_STATE_POSITION =
        keccak256("catalist.ValidatorsExitBusOracle.dataProcessingState");

    ICatalistLocator internal immutable LOCATOR;
    ...
    ///
    function getLastRequestedValidatorIndices(
        uint256 moduleId,
        uint256[] calldata nodeOpIds
        ...
        for (uint256 i = 0; i < nodeOpIds.length; ++i) {
            ...
            RequestedValidator
                memory validator = _storageLastRequestedValidatorIndices()[
                    nodeOpKey
                ];
            indices[i] = validator.requested
                ? int256(uint256(validator.index))
                : -1;
        }
        ...
        returns (ProcessingState memory result)
    {
        ...
        DataProcessingState memory procState = _storageDataProcessingState()
            .value;
    ...
    function _handleConsensusReport(
        ...
        if (
            ...
            state.requestsProcessed < state.requestsCount
        ) {
    ...
    function _handleConsensusReportData(ReportData calldata data) internal {
        ...
        TOTAL_REQUESTS_PROCESSED_POSITION.setStorageUint256(
            TOTAL_REQUESTS_PROCESSED_POSITION.getStorageUint256() +
                data.requestsCount
        );
    }
    ...
    function _processExitRequestsList(bytes calldata data) internal {
        ...
        while (offset < offsetPastEnd) {
            ...
            if (nodeOpKey != lastNodeOpKey) {
                ...
                lastRequestedVal = _storageLastRequestedValidatorIndices()[
                    nodeOpKey
                ];
            ...
            if (
                lastRequestedVal.requested && valIndex <= lastRequestedVal.index
            ) {
    ...
    function _storageLastRequestedValidatorIndices()
        internal
        pure
        returns (mapping(uint256 => RequestedValidator) storage r)
    {
    ...
    function _storageDataProcessingState()
        internal
        pure
        returns (StorageDataProcessingState storage r)
    {
