...
contract AccountingOracle is BaseOracle {
    ...
    error CatalistLocatorCannotBeZero();
    error AdminCannotBeZero();
    error LegacyOracleCannotBeZero();
    error CatalistCannotBeZero();
    ...
    bytes32 internal constant EXTRA_DATA_PROCESSING_STATE_POSITION =
        keccak256("catalist.AccountingOracle.extraDataProcessingState");

    address public immutable CATALIST;
    ICatalistLocator public immutable LOCATOR;
    ...
    constructor(
        address catalistLocator,
        address catalist,
        address legacyOracle,
        uint256 secondsPerSlot,
        uint256 genesisTime
    ) BaseOracle(secondsPerSlot, genesisTime) {
        if (catalistLocator == address(0)) revert CatalistLocatorCannotBeZero();
        if (legacyOracle == address(0)) revert LegacyOracleCannotBeZero();
        if (catalist == address(0)) revert CatalistCannotBeZero();
        LOCATOR = ICatalistLocator(catalistLocator);
        CATALIST = catalist;
    ...
    struct ReportData {
        ...
        /// via Catalist as observed at the reference slot.
        uint256 numValidators;
        /// @dev Cumulative balance of all Catalist validators on the consensus layer
        ...
        /// @dev The ACE balance of the Catalist withdrawal vault as observed at the reference slot.
        uint256 withdrawalVaultBalance;
        /// @dev The ACE balance of the Catalist execution layer rewards vault as observed
        ...
        returns (ProcessingState memory result)
    {
        ...
        ExtraDataProcessingState
            memory extraState = _storageExtraDataProcessingState().value;
    ...
    function _checkOracleMigration(
        ...
        (uint256 initialEpoch, uint256 epochsPerFrame) = IConsensusContract(
            consensusContract
        ).getFrameConfig();
        ...
        (
            ...
            uint256 secondsPerSlot,
            uint256 genesisTime
        ) = IConsensusContract(consensusContract).getChainConfig();

        {
            ...
            ) = ILegacyOracle(legacyOracle).getBeaconSpec();
            if (
        ...
        uint256 legacyProcessedEpoch = ILegacyOracle(legacyOracle)
    ...
    function _handleConsensusReport(
        ...
        ExtraDataProcessingState
            memory state = _storageExtraDataProcessingState().value;
        if (
    ...
    function _handleConsensusReportData(
        ReportData calldata data,
        uint256 prevRefSlot
    ) internal {
        ...
        ICatalist(CATALIST).handleOracleReport(
            GENESIS_TIME + data.refSlot * SECONDS_PER_SLOT,
    ...
    function _processStakingRouterExitedValidatorsByModule(
        ...
        uint256 newlyExitedValidatorsCount = stakingRouter
            .updateExitedValidatorsCountByStakingModule(
                stakingModuleIds,
                numExitedValidatorsByStakingModule
            );

        uint256 exitedValidatorsRatePerDay = (newlyExitedValidatorsCount *
            (1 days)) / (SECONDS_PER_SLOT * slotsElapsed);
    ...
    function _submitReportExtraDataEmpty() internal {
        ExtraDataProcessingState
            memory procState = _storageExtraDataProcessingState().value;
