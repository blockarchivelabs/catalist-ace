...
library SigningKeys {
    ...
    function getKeyOffset(
        ...
        return
            uint256(
                keccak256(
                    abi.encodePacked(_position, _nodeOperatorId, _keyIndex)
                )
            );
    }
    ...
    function saveKeysSigs(
        ...
        require(
            _keysCount > 0 && _startIndex.add(_keysCount) <= UINT64_MAX,
            "INVALID_KEYS_COUNT"
        );
        require(
    ...
    function removeKeysSigs(
        ...
        require(
            _keysCount > 0 &&
                _startIndex.add(_keysCount) <= _totalKeysCount &&
                _totalKeysCount <= UINT64_MAX,
            "INVALID_KEYS_COUNT"
        );
        ...
        for (uint256 i = _startIndex + _keysCount; i > _startIndex; ) {
            ...
            if (i < _totalKeysCount) {
                lastOffset = _position.getKeyOffset(
                    _nodeOperatorId,
                    _totalKeysCount - 1
                );
    ...
    function loadKeysSigs(
        ...
        uint256 curOffset;
        for (uint256 i; i < _keysCount; ) {
            curOffset = _position.getKeyOffset(
                _nodeOperatorId,
                _startIndex + i
            );
            assembly {
    ...
    function initKeysSigsBuf(
        ...
        return (
            new bytes(_count.mul(PUBKEY_LENGTH)),
            new bytes(_count.mul(SIGNATURE_LENGTH))
        );
    }
}
