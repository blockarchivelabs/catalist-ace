...
library StakeLimitUnstructuredStorage {
     ...
     */
    function getStorageStakeLimitStruct(
        bytes32 _position
    ) internal view returns (StakeLimitState.Data memory stakeLimit) {
        uint256 slotValue = _position.getStorageUint256();

        stakeLimit.prevStakeBlockNumber = uint32(
            slotValue >> PREV_STAKE_BLOCK_NUMBER_OFFSET
        );
        stakeLimit.prevStakeLimit = uint96(
            slotValue >> PREV_STAKE_LIMIT_OFFSET
        );
        stakeLimit.maxStakeLimitGrowthBlocks = uint32(
            slotValue >> MAX_STAKE_LIMIT_GROWTH_BLOCKS_OFFSET
        );
    ...
    function setStorageStakeLimitStruct(
        ...
        _position.setStorageUint256(
            (uint256(_data.prevStakeBlockNumber) <<
                PREV_STAKE_BLOCK_NUMBER_OFFSET) |
                (uint256(_data.prevStakeLimit) << PREV_STAKE_LIMIT_OFFSET) |
                (uint256(_data.maxStakeLimitGrowthBlocks) <<
...
library StakeLimitUtils {
     ...
     */
    function calculateCurrentStakeLimit(
        StakeLimitState.Data memory _data
        ...
        uint256 projectedLimit = _data.prevStakeLimit +
            blocksPassed *
            stakeLimitIncPerBlock;
     ...
     */
    function _constGasMin(
        uint256 _lhs,
        uint256 _rhs
    ) internal pure returns (uint256 min) {
        uint256 lhsIsLess;
        assembly {
