 ...
 * which equals to:
 *
 *   shares[account] * _getTotalPooledAce() / _getTotalShares()
 *
 * For example, assume that we have:
 *
 *   _getTotalPooledAce() -> 10 ACE
 *   sharesOf(user1) -> 100
 *   sharesOf(user2) -> 400
 *
 * Therefore:
 *
 *   balanceOf(user1) -> 2 tokens which corresponds 2 ACE
 *   balanceOf(user2) -> 8 tokens which corresponds 8 ACE
 *
 * Since balances of all token holders change when the amount of total pooled Ace
 * changes, this token cannot fully implement ERC20 standard: it only emits `Transfer`
 * events upon explicit transfer between holders. In contrast, when total amount of
 * pooled Ace increases, no `Transfer` events are generated: doing so would require
 ...
 * in `Catalist.sol` and might be called by an account with the `PAUSE_ROLE` assigned by the
...
contract BACE is IERC20, Pausable {
     ...
     *
     * The Catalist protocol is built on top of Aragon and uses the Unstructured Storage pattern
     * for value types:
     *
    ...
    function name() external pure returns (string) {
        return "Bond Ace JaeBal DuaeRa Zzom";
    }
     ...
     *
     * @dev Always equals to `_getTotalPooledAce()` since token amount
     * is pegged to the total amount of Ace controlled by the protocol.
     */
     ...
     * total Ace controlled by the protocol. See `sharesOf`.
     */
    function balanceOf(address _account) external view returns (uint256) {
        return getPooledAceByShares(_sharesOf(_account));
    }
    ...
    /**
     * @return the amount of shares that corresponds to `_aceAmount` protocol-controlled Ace.
     */
    function getSharesByPooledAce(
        uint256 _aceAmount
    ) public view returns (uint256) {
        return _aceAmount.mul(_getTotalShares()).div(_getTotalPooledAce());
    }

    /**
     * @return the amount of Ace that corresponds to `_sharesAmount` token shares.
     */
    function getPooledAceByShares(
        uint256 _sharesAmount
    ) public view returns (uint256) {
        return _sharesAmount.mul(_getTotalPooledAce()).div(_getTotalShares());
    }
    ...
    function transferShares(
        ...
        uint256 tokensAmount = getPooledAceByShares(_sharesAmount);
        _emitTransferEvents(
            msg.sender,
            _recipient,
            tokensAmount,
     ...
     * - the caller must have allowance for `_sender`'s tokens of at least `getPooledAceByShares(_sharesAmount)`.
     * - the contract must not be paused.
     *
     * @dev The `_sharesAmount` argument is the amount of shares, not tokens.
     */
    ...
    /**
     * @return the total amount (in wei) of Ace controlled by the protocol.
    ...
    function _transfer(
        ...
        uint256 _sharesToTransfer = getSharesByPooledAce(_amount);
    ...
    function _burnShares(
        ...
        uint256 preRebaseTokenAmount = getPooledAceByShares(_sharesAmount);
        ...
        uint256 postRebaseTokenAmount = getPooledAceByShares(_sharesAmount);
    ...
    function _emitTransferAfterMintingShares(
        ...
        _emitTransferEvents(
            address(0),
            _to,
            getPooledAceByShares(_sharesAmount),
            _sharesAmount
        );
    }
