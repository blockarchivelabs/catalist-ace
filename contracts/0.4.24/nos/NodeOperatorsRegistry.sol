...
contract NodeOperatorsRegistry is AragonApp, Versioned {
    ...
    event NodeOperatorPenalized(
        address indexed recipientAddress,
        uint256 sharesPenalizedAmount
    );
    ...
    bytes32 public constant MANAGE_SIGNING_KEYS =
    ...
    function finalizeUpgrade_v2(
        ...
        for (
            uint256 nodeOperatorId;
            nodeOperatorId < totalOperators;
            ++nodeOperatorId
        ) {
            signingKeysStats = _loadOperatorSigningKeysStats(nodeOperatorId);
            vettedSigningKeysCountBefore = signingKeysStats.get(
                TOTAL_VETTED_KEYS_COUNT_OFFSET
            );
            totalSigningKeysCount = signingKeysStats.get(
                TOTAL_KEYS_COUNT_OFFSET
            );
            depositedSigningKeysCount = signingKeysStats.get(
            ...
            operatorTargetStats = _loadOperatorTargetValidatorsStats(
                nodeOperatorId
            );
            operatorTargetStats.set(
                MAX_VALIDATORS_COUNT_OFFSET,
                vettedSigningKeysCountAfter
            );
            _saveOperatorTargetValidatorsStats(
                nodeOperatorId,
                operatorTargetStats
            );
    ...
    function _initialize_v2(
        ...
        CATALIST_LOCATOR_POSITION.setStorageAddress(_locator);
    ...
    function addNodeOperator(
        ...
        ACTIVE_OPERATORS_COUNT_POSITION.setStorageUint256(
    ...
    function deactivateNodeOperator(
        ...
        Packed64x4.Packed
            memory signingKeysStats = _loadOperatorSigningKeysStats(
                _nodeOperatorId
            );
        uint256 vettedSigningKeysCount = signingKeysStats.get(
            TOTAL_VETTED_KEYS_COUNT_OFFSET
        );
        uint256 depositedSigningKeysCount = signingKeysStats.get(
            TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
        );
    ...
    function setNodeOperatorStakingLimit(
        ...
        uint256 vettedSigningKeysCountBefore = signingKeysStats.get(
            TOTAL_VETTED_KEYS_COUNT_OFFSET
        );
        uint256 depositedSigningKeysCount = signingKeysStats.get(
            TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
        );
        uint256 totalSigningKeysCount = signingKeysStats.get(
            TOTAL_KEYS_COUNT_OFFSET
        );

        uint256 vettedSigningKeysCountAfter = Math256.min(
            totalSigningKeysCount,
            Math256.max(_vettedSigningKeysCount, depositedSigningKeysCount)
        );
    ...
    function _checkReportPayload(
        uint256 idsLength,
        uint256 countsLength
    ) internal pure returns (uint256 count) {
        count = idsLength / 8;
        require(
            countsLength / 16 == count &&
                idsLength % 8 == 0 &&
                countsLength % 16 == 0,
            "INVALID_REPORT_DATA"
        );
    ...
    function updateStuckValidatorsCount(
        ...
        for (uint256 i; i < nodeOperatorsCount; ) {
            ...
            assembly {
                nodeOperatorId := shr(
                    192,
                    calldataload(add(_nodeOperatorIdsOffset, mul(i, 8)))
                )
                validatorsCount := shr(
                    128,
                    calldataload(add(_stuckValidatorsCountsOffset, mul(i, 16)))
                )
                i := add(i, 1)
            }
    ...
    function updateExitedValidatorsCount(
        ...
        for (uint256 i; i < nodeOperatorsCount; ) {
            ...
            assembly {
                ...
                validatorsCount := shr(
                    128,
                    calldataload(add(_exitedValidatorsCountsOffset, mul(i, 16)))
                )
                i := add(i, 1)
            }
            _requireValidRange(nodeOperatorId < totalNodeOperatorsCount);
    ...
    function _updateExitedValidatorsCount(
        ...
        uint256 oldExitedValidatorsCount = signingKeysStats.get(
        ...
        uint256 depositedValidatorsCount = signingKeysStats.get(
            TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
        );
        uint256 stuckValidatorsCount = _loadOperatorStuckPenaltyStats(
            _nodeOperatorId
        ).get(STUCK_VALIDATORS_COUNT_OFFSET);

        // sustain invariant exited + stuck <= deposited
        assert(depositedValidatorsCount >= stuckValidatorsCount);
        _requireValidRange(
            _exitedValidatorsCount <=
                depositedValidatorsCount - stuckValidatorsCount
        );
        ...
        Packed64x4.Packed
            memory summarySigningKeysStats = _loadSummarySigningKeysStats();
        uint256 exitedValidatorsAbsDiff = Math256.absDiff(
            _exitedValidatorsCount,
            oldExitedValidatorsCount
        );
        if (_exitedValidatorsCount > oldExitedValidatorsCount) {
            summarySigningKeysStats.add(
                SUMMARY_EXITED_KEYS_COUNT_OFFSET,
                exitedValidatorsAbsDiff
            );
        } else {
            summarySigningKeysStats.sub(
                SUMMARY_EXITED_KEYS_COUNT_OFFSET,
                exitedValidatorsAbsDiff
            );
        }
    ...
    function updateTargetValidatorsLimits(
        ...
        Packed64x4.Packed
            memory operatorTargetStats = _loadOperatorTargetValidatorsStats(
                _nodeOperatorId
            );
        operatorTargetStats.set(
            IS_TARGET_LIMIT_ACTIVE_OFFSET,
            _isTargetLimitActive ? 1 : 0
        );
        operatorTargetStats.set(
            TARGET_VALIDATORS_COUNT_OFFSET,
            _isTargetLimitActive ? _targetLimit : 0
        );
        _saveOperatorTargetValidatorsStats(
            _nodeOperatorId,
            operatorTargetStats
    ...
    function _updateStuckValidatorsCount(
        ...
        Packed64x4.Packed
            memory stuckPenaltyStats = _loadOperatorStuckPenaltyStats(
                _nodeOperatorId
            );
        uint256 curStuckValidatorsCount = stuckPenaltyStats.get(
        ...
        uint256 exitedValidatorsCount = signingKeysStats.get(
            TOTAL_EXITED_KEYS_COUNT_OFFSET
        );
        uint256 depositedValidatorsCount = signingKeysStats.get(
            TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
        ...
        _requireValidRange(
            _stuckValidatorsCount <=
                depositedValidatorsCount - exitedValidatorsCount
        );

        uint256 curRefundedValidatorsCount = stuckPenaltyStats.get(
            REFUNDED_VALIDATORS_COUNT_OFFSET
        );
        if (
            _stuckValidatorsCount <= curRefundedValidatorsCount &&
            curStuckValidatorsCount > curRefundedValidatorsCount
        ) {
            stuckPenaltyStats.set(
    ...
    function _updateRefundValidatorsKeysCount(
        ...
        if (
            _refundedValidatorsCount >= curStuckValidatorsCount &&
            curRefundedValidatorsCount < curStuckValidatorsCount
        ) {
            stuckPenaltyStats.set(
                STUCK_PENALTY_END_TIMESTAMP_OFFSET,
                block.timestamp + getStuckPenaltyDelay()
    ...
    function _updateSummaryMaxValidatorsCount(
        uint256 _nodeOperatorId
    ) internal {
        (
            uint256 oldMaxSigningKeysCount,
            uint256 newMaxSigningKeysCount
        ) = _applyNodeOperatorLimits(_nodeOperatorId);
        ...
        uint256 maxSigningKeysCountAbsDiff = Math256.absDiff(
            newMaxSigningKeysCount,
            oldMaxSigningKeysCount
        );
        if (newMaxSigningKeysCount > oldMaxSigningKeysCount) {
            summarySigningKeysStats.add(
                SUMMARY_MAX_VALIDATORS_COUNT_OFFSET,
                maxSigningKeysCountAbsDiff
            );
        } else {
            summarySigningKeysStats.sub(
                SUMMARY_MAX_VALIDATORS_COUNT_OFFSET,
    ...
    function _invalidateReadyToDepositKeysRange(
        uint256 _indexFrom,
        uint256 _indexTo
    ) internal {
        _requireValidRange(
            _indexFrom <= _indexTo && _indexTo < getNodeOperatorsCount()
        );
        ...
        Packed64x4.Packed memory signingKeysStats;
        for (
            uint256 nodeOperatorId = _indexFrom;
            nodeOperatorId <= _indexTo;
            ++nodeOperatorId
        ) {
            signingKeysStats = _loadOperatorSigningKeysStats(nodeOperatorId);
    ...
    function obtainDepositData(
        ...
        require(
            allocatedKeysCount == _depositsCount,
        ...
        )
    {
        ...
        exitedSigningKeysCount = signingKeysStats.get(
            TOTAL_EXITED_KEYS_COUNT_OFFSET
        );
        depositedSigningKeysCount = signingKeysStats.get(
            TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
        );
        maxSigningKeysCount = operatorTargetStats.get(
            MAX_VALIDATORS_COUNT_OFFSET
        );

        // Validate data boundaries invariants here to not use SafeMath in caller methods
        assert(
            maxSigningKeysCount >= depositedSigningKeysCount &&
                depositedSigningKeysCount >= exitedSigningKeysCount
        );
    }

    function _applyNodeOperatorLimits(
        uint256 _nodeOperatorId
    )
        internal
        returns (uint256 oldMaxSigningKeysCount, uint256 newMaxSigningKeysCount)
    {
        ...
        newMaxSigningKeysCount = signingKeysStats.get(
            TOTAL_VETTED_KEYS_COUNT_OFFSET
        );
        ...
        if (!isOperatorPenaltyCleared(_nodeOperatorId)) {
            ...
            operatorTargetStats.get(IS_TARGET_LIMIT_ACTIVE_OFFSET) != 0
        ) {
        ...
        oldMaxSigningKeysCount = operatorTargetStats.get(
            MAX_VALIDATORS_COUNT_OFFSET
        );
        ...
        )
    {
        ...
        activeKeyCountsAfterAllocation = new uint256[](
    ...
    function _loadAllocatedSigningKeys(
        ...
        for (uint256 i; i < _nodeOperatorIds.length; ++i) {
            signingKeysStats = _loadOperatorSigningKeysStats(
                _nodeOperatorIds[i]
            );
            depositedSigningKeysCountBefore = signingKeysStats.get(
                TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
            );
            depositedSigningKeysCountAfter =
                signingKeysStats.get(TOTAL_EXITED_KEYS_COUNT_OFFSET) +
                _activeKeyCountsAfterAllocation[i];

            if (
                depositedSigningKeysCountAfter ==
                depositedSigningKeysCountBefore
            ) continue;
            ...
            //   depositedSigningKeysCount >= exitedSigningKeysCount
            assert(
                depositedSigningKeysCountAfter > depositedSigningKeysCountBefore
            );
        ...
        )
    {
        ...
        totalVettedValidators = uint64(
            signingKeysStats.get(TOTAL_VETTED_KEYS_COUNT_OFFSET)
        );
        ...
        )
    {
        ...
        for (uint256 operatorId; operatorId < nodeOperatorCount; ++operatorId) {
            ...
            uint256 totalExitedValidators = signingKeysStats.get(
                TOTAL_EXITED_KEYS_COUNT_OFFSET
            );
            uint256 totalDepositedValidators = signingKeysStats.get(
                TOTAL_DEPOSITED_KEYS_COUNT_OFFSET
            );

            // validate invariant to not use SafeMath.sub()
            assert(totalDepositedValidators >= totalExitedValidators);
            uint256 activeValidatorsCount = totalDepositedValidators -
        ...
        if (totalActiveValidatorsCount == 0)
        ...
        for (idx = 0; idx < activeCount; ++idx) {
            ...
            shares[idx] =
                shares[idx].mul(_totalRewardShares) /
    ...
    function _addSigningKeys(
        ...
        totalSigningKeysCount = SIGNING_KEYS_MAPPING_NAME.saveKeysSigs(
            _nodeOperatorId,
            totalSigningKeysCount,
            _keysCount,
            _publicKeys,
    ...
    function _removeUnusedSigningKeys(
        ...
        totalSigningKeysCount = SIGNING_KEYS_MAPPING_NAME.removeKeysSigs(
            _nodeOperatorId,
            _fromIndex,
            _keysCount,
            totalSigningKeysCount
    ...
    function getUnusedSigningKeyCount(
        ...
        return
            signingKeysStats.get(TOTAL_KEYS_COUNT_OFFSET).sub(
                signingKeysStats.get(TOTAL_DEPOSITED_KEYS_COUNT_OFFSET)
            );
    }
    ...
    function getSigningKey(
        ...
        bool[] memory keyUses;
        (key, depositSignature, keyUses) = getSigningKeys(
            _nodeOperatorId,
            _index,
            1
        );
        ...
        )
    {
        Packed64x4.Packed
            memory summarySigningKeysStats = _loadSummarySigningKeysStats();
        totalExitedValidators = summarySigningKeysStats.get(
            SUMMARY_EXITED_KEYS_COUNT_OFFSET
        );
        totalDepositedValidators = summarySigningKeysStats.get(
            SUMMARY_DEPOSITED_KEYS_COUNT_OFFSET
        );
        depositableValidatorsCount = summarySigningKeysStats
            .get(SUMMARY_MAX_VALIDATORS_COUNT_OFFSET)
            .sub(totalDepositedValidators);
    }
        ...
        )
    {
        ...
        isTargetLimitActive =
            operatorTargetStats.get(IS_TARGET_LIMIT_ACTIVE_OFFSET) != 0;
        targetValidatorsCount = operatorTargetStats.get(
            TARGET_VALIDATORS_COUNT_OFFSET
        );
        stuckValidatorsCount = stuckPenaltyStats.get(
            STUCK_VALIDATORS_COUNT_OFFSET
        );
        refundedValidatorsCount = stuckPenaltyStats.get(
            REFUNDED_VALIDATORS_COUNT_OFFSET
        );
        stuckPenaltyEndTimestamp = stuckPenaltyStats.get(
            STUCK_PENALTY_END_TIMESTAMP_OFFSET
        );
    ...
    function _getNodeOperatorValidatorsSummary(
        uint256 _nodeOperatorId
    )
        internal
        view
        ...
        )
    {
        ...
        (
            ...
            totalDepositedValidators,
            totalMaxValidators
        ) = _getNodeOperator(_nodeOperatorId);
    ...
    function _isOperatorPenalized(
        Packed64x4.Packed memory stuckPenaltyStats
    ...
    function isOperatorPenaltyCleared(
        uint256 _nodeOperatorId
    ) public view returns (bool) {
        Packed64x4.Packed
            memory stuckPenaltyStats = _loadOperatorStuckPenaltyStats(
        ...
        return
            !_isOperatorPenalized(stuckPenaltyStats) &&
    ...
    function getNodeOperatorIds(
        ...
        nodeOperatorIds = new uint256[](
            Math256.min(_limit, nodeOperatorsCount - _offset)
        );
    ...
    function _distributeRewards() internal returns (uint256 distributed) {
        ...
        if (toBurn > 0) {
            IBurner(getLocator().burner()).requestBurnShares(
                address(this),
                toBurn
            );
        }
    ...
    function getLocator() public view returns (ICatalistLocator) {
    ...
    function _loadSummarySigningKeysStats()
        internal
        view
        returns (Packed64x4.Packed memory)
    {
        return _nodeOperatorSummary.summarySigningKeysStats;
    }

    function _saveSummarySigningKeysStats(
        Packed64x4.Packed memory _val
    ) internal {
        _nodeOperatorSummary.summarySigningKeysStats = _val;
    }

    function _loadOperatorTargetValidatorsStats(
        uint256 _nodeOperatorId
    ) internal view returns (Packed64x4.Packed memory) {
        return _nodeOperators[_nodeOperatorId].targetValidatorsStats;
    }

    function _saveOperatorTargetValidatorsStats(
        uint256 _nodeOperatorId,
        Packed64x4.Packed memory _val
    ) internal {
    ...
    function _loadOperatorStuckPenaltyStats(
        uint256 _nodeOperatorId
    ) internal view returns (Packed64x4.Packed memory) {
        return _nodeOperators[_nodeOperatorId].stuckPenaltyStats;
    }

    function _saveOperatorStuckPenaltyStats(
        uint256 _nodeOperatorId,
        Packed64x4.Packed memory _val
    ) internal {
    ...
    function _loadOperatorSigningKeysStats(
        uint256 _nodeOperatorId
    ) internal view returns (Packed64x4.Packed memory) {
        return _nodeOperators[_nodeOperatorId].signingKeysStats;
    }

    function _saveOperatorSigningKeysStats(
        uint256 _nodeOperatorId,
        Packed64x4.Packed memory _val
    ) internal {
