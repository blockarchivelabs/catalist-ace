...
task('snapshot', 'Take a snapshot')
  ...
  .addOptionalParam('launchpad', 'Launch pad address')
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const wbACE = loader.WbACE.contract;
    ...
    logs.forEach((log) => {
      ...
      if (!balances[from].bACE) {
        balances[from].bACE = new BN(0);
      }
      if (!balances[to].bACE) {
        balances[to].bACE = new BN(0);
      }

      balances[from].bACE = balances[from].bACE.sub(sharesValue);
      balances[to].bACE = balances[to].bACE.add(sharesValue);
    });

    delete balances[ZERO_ADDRESS];
    delete balances[INITIAL_TOKEN_HOLDER];
    ...
    const wbACEdir = path.resolve(
      __dirname,
      '../lib/abi',
      `${loader.WbACE.name}.json`,
    );
    const wbACEfile = fs.readFileSync(wbACEdir, 'utf8');
    const wbACEabi = JSON.parse(wbACEfile);
    const wbACEiface = new ethers.utils.Interface(wbACEabi);

    const wbACEfilter = {
      address: loader.WbACE.address,
      fromBlock: 0,
      toBlock: taskArgs.block,
      topics: [wbACE.filters['Transfer']().topics],
    };

    const wbAceTransferLogs = await ethers.provider.getLogs(wbACEfilter);

    for (const log of wbAceTransferLogs) {
      const decodedData = wbACEiface.decodeEventLog(
        'Transfer',
        log.data,
        log.topics,
      );
      const from = decodedData.from;
      const to = decodedData.to;
      const value = new BN(decodedData.value.toString());
      ...
      if (!balances[from].wbACE) {
        balances[from].wbACE = new BN(0);
      }
      if (!balances[to].wbACE) {
        balances[to].wbACE = new BN(0);
      }

      if (taskArgs['launchpad'] && to === taskArgs['launchpad']) {
        continue;
      }
      balances[from].wbACE = balances[from].wbACE.sub(value);
      balances[to].wbACE = balances[to].wbACE.add(value);
    }

    delete balances[ZERO_ADDRESS];
    delete balances[INITIAL_TOKEN_HOLDER];

    const totalPooledAce = await catalist.getTotalPooledAce(TX_OPTIONS);
    const TOTAL_POOLED_ACE = new BN(totalPooledAce.toString());

    const totalShares = await catalist.getTotalShares(TX_OPTIONS);
    const TOTAL_SHARES = new BN(totalShares.toString());

    const WRAP_RATIO = await wbACE.bAcePerToken(TX_OPTIONS);
    const WRAP_RATIO_BN = new BN(WRAP_RATIO.toString());

    for (const account of Object.keys(balances)) {
      if (account !== ZERO_ADDRESS && account !== INITIAL_TOKEN_HOLDER) {
        const balance = balances[account].bACE
          .mul(TOTAL_POOLED_ACE)
          .div(TOTAL_SHARES);

        balances[account].bACE = +ethers.utils.formatEther(balance.toString());
        if (balances[account].bACE == '1E-18') {
          balances[account].bACE = 0;
        }
        balances[account].wbACE = +ethers.utils.formatEther(
          balances[account].wbACE.toString(),
        );
      }
    }
    ...
    function jsonToCsv(jsonData) {
      const csvRows = [];
      const headers = ['Address', 'bACE', 'wbACE'];
      ...
      for (const address in jsonData) {
        const values = [
          address,
          jsonData[address].bACE,
          jsonData[address].wbACE,
        ];
    ...
    function saveCsvToFile(jsonData, filename) {
      const csvData = jsonToCsv(jsonData);
      fs.writeFileSync(filename, csvData);
    }

    saveCsvToFile(balances, 'balances.csv');
  });
...
task('node-operator-summary', 'Get node operator summary')
  ...
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const NODE_OPERATOR_ID =
      taskArgs['operator'] > 0 ? taskArgs['operator'] : 0;
    const block = +taskArgs['block'] || 'latest';

    console.log();
    console.log('- node operator id:', NODE_OPERATOR_ID);
    console.log('- block:', block);
    // block 의 timestamp 구하는 코드
    const blockData = await ethers.provider.getBlock(block);
...
task('balance', 'Get account balance')
  ...
  .setAction(async (taskArgs, { ethers }) => {
    ...
    const wbACE = await loader.WbACE.contract.balanceOf(ACCOUNT, GAS_INFO);
    console.log('- wbACE:', +ethers.utils.formatEther(wbACE));
  });
